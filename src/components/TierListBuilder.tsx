      ...
      >
        ...
        <div data-export-board className="border border-black/60 bg-black/60">
          {tiers.map((tier, i) => (
            <div key={tier.key} className="border-b border-black/60 last:border-b-0">
              <div className="flex min-h-16 sm:min-h-20">
                <div
                  className="group/label relative flex w-14 shrink-0 items-center justify-center break-words p-1 text-center font-bold leading-tight text-black sm:w-24 sm:p-2"
                  style={{
                ...
                <SortableContext items={containers[tier.key] ?? []} strategy={rectSortingStrategy}>
                  <DroppableZone id={tier.key} className="flex flex-1 flex-wrap content-start items-start bg-surface">
                    ...
                    <button
                      ...
                      className={`grid h-16 w-9 place-items-center text-xl transition-colors sm:h-20 sm:w-10 ${
        ...
        <div className="border border-edge bg-surface">
          ...
          <SortableContext items={visiblePool} strategy={rectSortingStrategy}>
            <DroppableZone
              id="pool"
              className="grid max-h-[420px] min-h-16 grid-cols-[repeat(auto-fill,minmax(8rem,1fr))] content-start overflow-y-auto sm:min-h-20 sm:grid-cols-[repeat(auto-fill,minmax(9rem,1fr))]"
            >
        ...
        <DragOverlay dropAnimation={dropAnimation}>
          {activeId && modelById.has(activeId) ? (
            <div className="w-32 scale-110 cursor-grabbing sm:w-36 shadow-[0_12px_32px_rgba(0,0,0,0.7)] ring-2 ring-white/40">
