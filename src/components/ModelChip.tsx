...
export default function ModelChip({ model }: { model: Model }) {
  ...
  return (
    <div
      className="relative flex h-16 w-full select-none items-center gap-2 bg-surface-2 px-2 outline outline-1 outline-black/60 sm:h-20 sm:px-2.5"
      title={`${model.name} — ${model.vendor}${isThinking ? " (thinking mode)" : ""}`}
    >
      <VendorLogo vendorSlug={model.vendor_slug} className="h-6 w-6 shrink-0 sm:h-8 sm:w-8" />
